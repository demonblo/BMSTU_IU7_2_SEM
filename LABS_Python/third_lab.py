...
def info(event):
    information = 'Данная программа формирует таблицу корней функции f(x) = sin(x) - 0.5 на отрезке [a, b] с шагом h и точностью eps, которые вводятся пользователем.\nРазработано студентом ИУ7-22Б\nБлохиным Дмитрием.'
    showinfo('Информация о программе', information)
            ...
            while right <= b:
                ...
                if f(left) <= 0 and f(right) >= 0 or f(left) >= 0 and f(right) <= 0:
                    ...
                    while counter_of_it < max_it and abs(cur - prev) > eps:
                        ...
                        prev = x[i - 1]
                        counter_of_it += 1
                        if f_pr(left) >= 0 and f_pr(right) <= 0 or f_pr(left) <= 0 and f_pr(right) >= 0 or f_sec_pr(left) > 0 and f_sec_pr(right) < 0 or f_sec_pr(left) < 0 and f_sec_pr(right) > 0:
                            code = 1

                    if counter_of_it == max_it and abs(cur - prev) > eps:
                        code = 2
                if code != 10 and code != 1:
                    otrezok_out = '[' + str(left) + ' ,' + ' ' + str(right) + ']'
                    x_out = str(round(cur, 5))
                    f_out = str(round(f(cur), 5))
                    if code == 2:
                        x_out = '-'
                        f_out = '-'
